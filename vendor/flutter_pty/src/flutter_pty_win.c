...
// The Dart side already places `executable` at arguments[0] (Unix execv
// convention). On Windows, CreateProcessW takes a single command-line
// string where the first token is the program name, so we should NOT also
// prepend `executable` separately — that would duplicate it. We iterate
// only `arguments` (which already starts with the program name).
