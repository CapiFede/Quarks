...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  MediaKitPlayer(super.id) {
    _player = Player(
      ...
      // osc=true tells media_kit NOT to set the MPV `osc` property.
      // In audio-only builds (media_kit_libs_windows_audio) the OSC module
      // is not compiled in, so any attempt to set that property fails with
      // "property not found". Since there is no video output, OSC is unused.
      osc: JustAudioMediaKit.osc,
    ...
    _streamSubscriptions = [
      ...
      _player.stream.log.listen((event) {
        // lavf reports a non-fatal "Failed to create file cache" when the
        // optional on-disk seek-cache can't be created. Playback continues
        // normally, so suppress the noise.
        if (event.prefix == 'lavf' &&
            event.text.contains('Failed to create file cache')) return;
        // ignore: avoid_print
